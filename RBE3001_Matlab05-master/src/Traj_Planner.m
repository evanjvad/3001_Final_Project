classdef Traj_Planner
    ...
    methods
        ...
        %%This function will utilize the quintic trajectory implemented in
        %%lab 3 for robot movement in the xy-plane. For documentation of
        %%this process, refer to lab 3.
        ...
        %%This function will utilize the quintic trajectory implemented in
        %%lab 3 for robot movement in the z-plane. For documentation of
        %%this process, refer to lab 3.  
        function move_To_Z(obj, end_Point, isGrabbing)
            ...
            %Makes sure the gripper is open and primed
            if isGrabbing
                obj.pp.openGripper();
            else
                obj.pp.closeGripper();
        ...
        function placement(obj, start_Point, end_Point)
           %% Movement to the first position (over ball and on ball)
            X_Point = start_Point(1)+4;
            Y_Point = start_Point(2);
            xy_move = [X_Point, Y_Point, obj.xy_Height];
            move_To_Z = [X_Point, Y_Point, obj.z_Adjustment_Height];
            obj.move_To_Position(xy_move);
            obj.move_To_Z(move_To_Z, true);
            
            %% Move to the second position (over end pos and on end pos)
        ...
        function home_Orientation(obj)
           goToHome = obj.pp.fk3001([0 0 0]);
           points = [goToHome(1,4) goToHome(2,4) goToHome(3,4)];
